...
/**
 * BuildMLSTree() - creates an MLS tree
 *
 * @param	conf1				Eastern or Western conference
 * @param	conf2			    Eastern or Western conference
 *
 ...
 *
 */
void BuildMLSTrees(MLStreePtr_t east_conf, MLStreePtr_t west_conf){
    FILE *titlefile;
    char title[256];
 ...
 */
 MLStreePtr_t createMLStree(MLSconf_t conf, int numTeams, int numPlayoffTeams) {
	...
	temp->root = NULL; //ALREADY NULL, just confirming as a precaution.
...
TreeNodePtr_t insert(MLStreePtr_t tree, TeamInfo_t info) {
    ...
    //Note this was originally implemented but due to the nodes not actually storing data, the malloc below is not required or needed.
    //Keeping for future reference and debug.
 ...
 */
 void printStandings(MLStreePtr_t tree) {
    ...
    //Could potentially isolate the code below using the formating as used above and by examples in Roy's code.
